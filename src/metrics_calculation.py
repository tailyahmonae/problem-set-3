...
def calculate_sklearn_metrics(model_pred_df, genre_list):
    ...
    for _, row in model_pred_df.iterrows():
        pred_row = [1 if genre == row['predicted'] else 0 for genre in genre_list]
        true_row = [1 if genre in eval(row['actual genres']) else 0 for genre in genre_list]
    ...
    # precision, recall, f1 for each genre
